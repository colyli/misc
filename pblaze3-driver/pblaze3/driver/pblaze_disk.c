...
#else
void pblaze_disk_end_unalign_io(struct bio *bio, int error)
#endif
{
	int i;
	struct bio *orig_bio;
...
#else
void pblaze_disk_end_loose_io(struct bio *bio, int error)
#endif
{
	unsigned short i;
	struct bio_vec *bio_vec;
...
#else
void pblaze_disk_make_request(struct request_queue *q, struct bio *bio_orig)
#endif
{
	int error;
	...
	if (pb_is_has_data_bio(bio)) {
		...
		DPRINTK(ERR, "Unknowed io type, flags:%lu, rw:%lu, S:%lu, L:%lu\n",
			bio->bi_flags, bio->bi_rw, (unsigned long)start, (unsigned long)len);
		error = -EOPNOTSUPP;
		goto failed_endio;
	}
